...
- Each `STORYBOARD.json.images[]` entry should keep one storyboard image scoped to one shot anchor, using `frameType`, `prompt`, optional `camera`, and nullable `imagePath`. Storyboard prompts should be authored directly for the fast storyboard model as explicit FLUX-style visual prompts rather than abstract goals. Storyboard `camera` should use `CAMERA_VOCABULARY.json` ids with the shape `shotSize`, `cameraPosition`, `cameraAngle`. If the board does not imply a stronger framing choice, default storyboard `camera` to `medium-shot`, `eye-level`, `level-angle`. Shot ids and storyboard image ids are derived from board order, not stored.
...
- When keyframes are rendered, the referenced storyboard image should be treated as the upstream visual reference for staging and intent.
