...
- Before writing or revising keyframe sidecar JSON files, character-sheet sidecar JSON files, or `SHOT-PROMPTS.json`, first read `workspace/CONFIG.json` and `MODEL_PROMPTING_GUIDE.md`.
...
- When writing `SHOT-PROMPTS.json`, set each prompt entry's `model` to `workspace/CONFIG.json.videoModel`.
...
- `SHOT-PROMPTS.json` should reference the keyframe IDs that anchor the generated shot.
