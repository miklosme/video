...
- Before writing or revising keyframe sidecar JSON files, character-sheet sidecar JSON files, shot sidecar JSON files, or `SHOT-PROMPTS.json`, first read `workspace/CONFIG.json` and `MODEL_PROMPTING_GUIDE.md`.
...
- `SHOT-PROMPTS.json` is planning-only and must use the exact shape: `shotId`, `status`, `videoPath`, `keyframeIds`.
...
- `SHOT-PROMPTS.json` should reference the keyframe IDs that anchor the generated shot and the canonical `videoPath` for its MP4 output.
