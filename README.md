...
- `bun generate-storyboard.ts` syncs missing `workspace/STORYBOARD/*.png` files from `workspace/STORYBOARD.json`, renders one storyboard image at a time with `workspace/CONFIG.json.fastImageModel`, uses `workspace/CONFIG.json.agentModel` to rewrite storyboard prompts for model-specific flows such as FLUX Klein, keeps image-local retained versions under `workspace/STORYBOARD/HISTORY/`, and preserves idempotent skip behavior when the public PNG already exists.
