...
- By default, plan one `start` and one `end` keyframe per storyboard shot. When the opening and closing frame would be effectively the same, use a deliberate one-anchor plan with only `start` or only `end`.
...
- `bun generate-keyframes.ts` syncs missing `workspace/KEYFRAMES/**/*.png` files from their sidecar JSON files, `workspace/KEYFRAMES.json`, and `workspace/SHOTS.json`, attaches the storyboard review board plus continuity references, includes the same-shot start frame for end-frame generation only when that start frame is planned, renders `workspace/CONFIG.json.variantCount` variants in sequence when a stable public keyframe is missing, stores earlier variants in `HISTORY/`, and keeps the last new variant at the public PNG path.
