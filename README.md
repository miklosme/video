...
- `workspace/STORYBOARD.json` is the canonical storyboard plan. It stores an ordered `images` array where each image owns `frameType`, `goal`, nullable `imagePath`, and optional `references`. Shot ids and storyboard image ids are derived from board order.
