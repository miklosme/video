...
- `workspace/STORYBOARD/STORYBOARD.json` is the canonical storyboard plan. It stores an ordered `images` array where each image owns `frameType`, `prompt`, optional `camera`, and nullable `imagePath`. Storyboard `camera` uses `CAMERA_VOCABULARY.json` ids with the still-image shape `shotSize`, `cameraPosition`, `cameraAngle`. Shot ids and storyboard image ids are derived from board order.
