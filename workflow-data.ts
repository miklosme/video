...
export const PHASES = ['concept', 'story', 'storyboard', 'keyframes', 'prompts', 'revision'] as const;
...
function expectEnum<T extends readonly string[]>(value: unknown, allowed: T, context: string): T[number] {
...
function parseProjectData(value: unknown): ProjectData {
  ...
  const targetVideoLengthSeconds = expectObject(object.targetVideoLengthSeconds, 'PROJECT.json.targetVideoLengthSeconds');
  ...
  return {
    ...
    targetEmotionalEffect: expectString(object.targetEmotionalEffect, 'PROJECT.json.targetEmotionalEffect'),
    currentPhase: expectEnum(object.currentPhase, PHASES, 'PROJECT.json.currentPhase'),
    targetModels: {
      keyframe: parseNullableModelSelection(targetModels.keyframe, 'PROJECT.json.targetModels.keyframe'),
      video: parseNullableModelSelection(targetModels.video, 'PROJECT.json.targetModels.video'),
      comparison: expectArray(targetModels.comparison, 'PROJECT.json.targetModels.comparison').map((entry, index) =>
        parseModelSelection(entry, `PROJECT.json.targetModels.comparison[${index}]`),
      ),
    },
    workingAssumptions: expectStringArray(object.workingAssumptions, 'PROJECT.json.workingAssumptions'),
  };
}
...
function parseStoryboardShot(value: unknown, context: string): StoryboardShot {
  ...
  return {
    ...
    continuityDependencies: expectStringArray(object.continuityDependencies, `${context}.continuityDependencies`),
...
function parseStoryboardData(value: unknown): StoryboardData {
  ...
  return {
    ...
    sequenceOverview: {
      targetRuntimeSeconds: expectNumber(sequenceOverview.targetRuntimeSeconds, 'STORYBOARD.json.sequenceOverview.targetRuntimeSeconds'),
      sceneCount: expectNumber(sequenceOverview.sceneCount, 'STORYBOARD.json.sequenceOverview.sceneCount'),
      shotCount: expectNumber(sequenceOverview.shotCount, 'STORYBOARD.json.sequenceOverview.shotCount'),
      structure: expectString(sequenceOverview.structure, 'STORYBOARD.json.sequenceOverview.structure'),
      escalationRule: expectString(sequenceOverview.escalationRule, 'STORYBOARD.json.sequenceOverview.escalationRule'),
    },
...
function parseReferenceCollectionTarget(value: unknown, context: string): ReferenceCollectionTarget {
  const object = expectObject(value, context);
...
function parseReferenceAsset(value: unknown, context: string): ReferenceAsset {
  ...
  return {
    ...
    shotId: object.shotId === undefined ? undefined : expectString(object.shotId, `${context}.shotId`),
    frameType:
      object.frameType === undefined ? undefined : expectEnum(object.frameType, FRAME_TYPES, `${context}.frameType`),
  };
}
...
function parseReferencesData(value: unknown): ReferencesData {
  ...
  return {
    ...
    collectionTargets: expectArray(object.collectionTargets, 'REFERENCES.json.collectionTargets').map((entry, index) =>
...
function parsePromptPackShot(value: unknown, context: string): PromptPackShot {
  ...
  return {
    ...
    approvedKeyframeAssetIds: expectStringArray(object.approvedKeyframeAssetIds, `${context}.approvedKeyframeAssetIds`),
    notes: expectString(object.notes, `${context}.notes`),
  };
}
...
function parsePromptPackData(value: unknown): PromptPackData {
  ...
  return {
    ...
    targetVideoModel: parseNullableModelSelection(object.targetVideoModel, 'PROMPT-PACK.json.targetVideoModel'),
...
function parseEditSelection(value: unknown, context: string): EditSelection {
  ...
  return {
    ...
    approvedGenerationId: expectNullableString(object.approvedGenerationId, `${context}.approvedGenerationId`),
    notes: expectString(object.notes, `${context}.notes`),
  };
}
...
function parseTestCase(value: unknown, context: string): TestCase {
  ...
  return {
    ...
    acceptanceCriteria: expectStringArray(object.acceptanceCriteria, `${context}.acceptanceCriteria`),
  };
}
...
function parseGenerationLogEntry(value: unknown, context: string): GenerationLogEntry {
  ...
  return {
    ...
    settings: {
      ...
      safetyFilterLevel: expectString(settings.safetyFilterLevel, `${context}.settings.safetyFilterLevel`),
    },
    ...
    frameType: object.frameType === null ? null : expectEnum(object.frameType, FRAME_TYPES, `${context}.frameType`),
...
async function readJsonFile<T>(fileName: string, parser: (value: unknown) => T, cwd = process.cwd()): Promise<T> {
...
export async function loadGenerationLog(cwd = process.cwd()) {
  ...
  return lines.map((line, index) => parseGenerationLogEntry(JSON.parse(line), `GENERATION-LOG.jsonl line ${index + 1}`));
}

export async function loadWorkflowData(cwd = process.cwd()) {
  const [project, storyboard, references, keyframes, promptPack, todo, qc, edit, tests, generationLog] = await Promise.all([
    loadProject(cwd),
    loadStoryboard(cwd),
    loadReferences(cwd),
    loadKeyframes(cwd),
...
export function validateWorkflowConsistency(data: Awaited<ReturnType<typeof loadWorkflowData>>) {
  ...
  const actualShotCount = data.storyboard.scenes.reduce((total, scene) => total + scene.shots.length, 0);
  ...
  for (const asset of data.references.assets) {
    if (asset.shotId && !storyboardShotIds.has(asset.shotId)) {
      throw new Error(`REFERENCES.json asset ${asset.assetId} references unknown shotId ${asset.shotId}.`);
    }
  }
  ...
  for (const review of data.qc.reviews) {
    if (!storyboardShotIds.has(review.shotId)) {
      throw new Error(`QC.json review ${review.reviewId} references unknown shotId ${review.shotId}.`);
    }
  }
}
