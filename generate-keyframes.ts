...
function parseArgs(): GenerateKeyframesArgs {
  ...
  if (frameTypeArg && !FRAME_TYPES.includes(frameTypeArg as FrameType)) {
    throw new Error(`Invalid --frame-type "${frameTypeArg}". Expected one of: ${FRAME_TYPES.join(', ')}.`);
  }
...
async function main() {
  ...
  if (selectedShots.length === 0) {
    throw new Error(`No keyframe shot matched${shotId ? ` shot ${shotId}` : ' the provided filters'}.`);
  }
